...
module Tree = struct
  ...
  let to_char_sequence t =
    let f (((x, xs) as xxs), xpos) : _ Sequence.Step.t =
      ...
      then Yield { value = x.[xpos]; state = xxs, xpos + 1 }
      else (
        match xs with
        | [] -> Done
        | y :: ys -> Skip { state = unroll y ys, 0 })
    in
...
let quickcheck_generator =
  ...
  Generator.weighted_union
    [ 1., Generator.return empty
    ; ( 100.
      , Generator.recursive_union
          [ Generator.map Generator.string_non_empty ~f:of_string ]
          ~f:(fun t ->
            [ Generator.map2 t t ~f:( ^ )
            ; Generator.map2 t Generator.string_non_empty ~f:(fun left right ->
                left ^ of_string right)
            ; Generator.map2 Generator.string_non_empty t ~f:(fun left right ->
...
let quickcheck_observer =
  Base_quickcheck.Observer.unmap Base_quickcheck.Observer.string ~f:to_string
;;
...
let quickcheck_shrinker =
  ...
  in
  Base_quickcheck.Shrinker.create (fun t ->
    match t.tree with
    | Base string ->
      Sequence.map
        ~f:of_string
        (Base_quickcheck.Shrinker.shrink Base_quickcheck.Shrinker.string string)
